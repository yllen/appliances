...
function plugin_appliances_createFirstAccess($ID) {

   $PluginAppliancesProfile = new PluginAppliancesProfile();
   if (!$PluginAppliancesProfile->GetfromDB($ID)) {
      $Profile = new Profile();
      $Profile->GetfromDB($ID);
      $name = $Profile->fields["name"];

      $PluginAppliancesProfile->add(array('id'          => $ID,
                                          'name'        => $name,
                                          'appliance'   => 'w',
                                          'open_ticket' => '1'));
   }
...
function plugin_appliances_createAccess($ID) {
...
function plugin_appliances_changeProfile() {
